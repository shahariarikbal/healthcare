            ...
            <div class="position-sticky bg-dark-subtle bottom-0 d-flex align-items-center justify-content-between border-top">
                <div class="dropup">
                    <div class="dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false">
                        <img class="img-fluid brand-sm-logo" src="{{ auth()->guard('account')->user()->avatar }}" alt="profile picture">
                        {{ auth()->guard('account')->user()->full_name }}
                        <ul class="dropdown-menu text-center">
                            <li>Settings</li>
                            <li>Profile</li>
                            <li>
                                <a href="{{ route('account.logout') }}"onclick="event.preventDefault();
                                document.getElementById('account-logout-form').submit();">Logout</a>
 
                                 <form id="account-logout-form" action="{{ route('account.logout') }}" method="POST" class="d-none">
                                     @csrf
                                 </form>
                            </li>
                        </ul>
